...
void treereader(Int_t *n_event_cntr, Int_t *n_event_fwd, Int_t *n_event_overlap, TString name,
	...
	TH1D *phi_fwd_1,  TH1D *phi_fwd_2,  TH1D *phi_fwd_3,
        ...
        Double_t eta_merged, Double_t pt_min_1, Double_t pt_min_2, Double_t FWD_weight,  Double_t pt_veto,
	bool check_trigger
        ){
...
if((pt_v_2.size() > 1)&&(pt_v_1.size() > 0)){
	...
	FWD_dijet = is_FWD_dijet(y_v_1, y_v_2, eta_merged);

	if((CNTR > 0)||(!check_trigger)){
		CNTR = 1;
		(*n_event_cntr)++;
		...
		if(FWD_dijet < 0){
			...
			dy_MN_histo->Fill(dy_MN,weight*CNTR);			
	   		for(int j = 0; j < dy_INCL.size(); j++){
				dy_cntr_minus_fwd->Fill(dy_INCL[j],weight*CNTR);
				dy_histo->Fill(dy_INCL[j],weight*CNTR);
			}
		}else{
			(*n_event_overlap)++;
		}
	...
	if(((FWD > 0)||(!check_trigger))&&(FWD_dijet > 0)){
		FWD = 1;
		(*n_event_fwd)++;
		dy_fwd_MN->Fill(dy_MN,weight*FWD);
		dy_MN_histo->Fill(dy_MN,weight*FWD*FWD_weight);
   		for(int j = 0; j < dy_INCL.size(); j++){
			dy_fwd->Fill(dy_INCL[j],weight*FWD);
			dy_histo->Fill(dy_INCL[j],weight*FWD*FWD_weight);
		}
   		for(int j = 0; j < pt_v_2.size(); j++){
