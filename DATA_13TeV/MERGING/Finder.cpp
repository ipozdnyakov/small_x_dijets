...
//////////////////
//FIND FORWARD DIJET
Int_t is_FWD_dijet(vector<Double_t> y_v_1, vector<Double_t> y_v_2, Double_t eta){

	Int_t dijet = -1;
	for(int i = 0; i < y_v_1.size(); i++){
		if(fabs(y_v_1[i]) > eta){
			for(int j = 0; j < y_v_2.size(); j++){
				if((fabs(y_v_2[j]) > eta)&&(y_v_2[j]*y_v_1[i] < 0)) dijet = 1;
			}
		}
	}
...
//////////////////
//FIND INCL DY
vector<Double_t> find_dy_INCL(vector<Double_t> y_v_1, Double_t pt_min_1, vector<Double_t> y_v_2, vector<Double_t> pt_v_2){

	vector<Double_t> dy_INCL;
	for(int i = 0; i < y_v_1.size(); i++){
		for(int j = y_v_1.size() - 1; j > i; j--){
			dy_INCL.push_back(fabs(y_v_1[i] - y_v_1[j]));
		}
	}

	for(int i = 0; i < y_v_2.size(); i++){
		if(pt_v_2[i] < pt_min_1){
			for(int j = 0; j < y_v_1.size(); j++){
				dy_INCL.push_back(fabs(y_v_2[i] - y_v_1[j]));
			}
		}
	}
